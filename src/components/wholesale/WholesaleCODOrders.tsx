 ...
 import { 
   useWholesaleCODOrders, 
   useAcceptWholesaleOrder, 
   useRejectWholesaleOrder, 
   useMarkWholesaleOrderReady,
   useRequestWholesaleRider
 } from '@/hooks/useWholesaleDelivery';
 import { useAvailableRiders } from '@/hooks/useDelivery';
 import { deliveryService, COD_ORDER_STATUSES } from '@/services/deliveryService';
 import { format } from 'date-fns';
 
 const WholesaleCODOrders: React.FC = () => {
   const { data: orders = [], isLoading, refetch } = useWholesaleCODOrders();
   const { data: riders = [] } = useAvailableRiders();
   const acceptOrder = useAcceptWholesaleOrder();
   const rejectOrder = useRejectWholesaleOrder();
   const markReady = useMarkWholesaleOrderReady();
   const requestRider = useRequestWholesaleRider();
     ...
     <div className="space-y-4">
       ...
       {orders.length === 0 ? (
         <Card>
           <CardContent className="p-8 text-center">
             ...
             <p className="text-muted-foreground">No pending COD orders from retailers</p>
           </CardContent>
         </Card>
       ) : (
             ...
             return (
               <Card key={order.id}>
                 ...
                 <CardContent className="space-y-4">
                   {/* Retailer Info */}
                   <div className="flex items-start gap-4 text-sm bg-muted/30 p-3 rounded-lg">
                     <div className="flex items-center gap-1">
                       <Building2 className="h-4 w-4 text-muted-foreground" />
                       <span className="font-medium">
                         {order.retailer?.pharmacy_name || order.retailer?.name || 'Unknown Retailer'}
                       </span>
                     </div>
                     <div className="flex items-center gap-1">
                       <MapPin className="h-4 w-4 text-muted-foreground" />
                       <span>{order.delivery_address || order.retailer?.address || 'No address'}</span>
                     </div>
                     <div className="flex items-center gap-1">
                       <Phone className="h-4 w-4 text-muted-foreground" />
                       <span>{order.delivery_phone || order.retailer?.phone || 'No phone'}</span>
                     </div>
                   </div>
       ...
       <Dialog open={riderDialogOpen} onOpenChange={setRiderDialogOpen}>
         <DialogContent>
           ...
           <div className="space-y-4">
             <p className="text-sm text-muted-foreground">
               Select a rider to deliver this order to the retailer. They will be notified immediately.
             </p>
 ...
 export default WholesaleCODOrders;